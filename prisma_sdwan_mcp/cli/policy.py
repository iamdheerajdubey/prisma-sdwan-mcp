...
vendor already makes at the family level). See docs/ION_CLI_RESEARCH.md.
...
from ..config import get_ion_max_commands
...
def validate_batch(commands: Iterable[object], max_commands: int | None = None) -> BatchDecision:
    if not isinstance(commands, (list, tuple)):
        return BatchDecision(False, (), "commands must be a list of strings")

    limit = get_ion_max_commands() if max_commands is None else max_commands
    if len(commands) > limit:
        return BatchDecision(
            False,
            (),
            f"batch of {len(commands)} commands exceeds the maximum of {limit}",
        )
